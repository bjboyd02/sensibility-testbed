 ...
 *  - Functions that search for java classes and methods or instantiate java objects
 *    and return the according JNI reference, e.g.:
 *    jclass, jmethodID, jobject, jstring
 *
 *  - Functions that call methods of a java object, convert the data returned from java
 *    to a Python Object using the C-Python API and return the PyObject pointer
 *    to the caller
 *
 ...
 *  - Use cjson library to decode string serialized complex Java data types (JSON format)
 *    to Python objects
 *
 *
 * Todo:
 *  - Consider name refactor, maybe something like java-c-python-glue would be more appropriate
 *
 *  - Consider restructuring, maybe JNI can be even more invisible to the calling C functions
 *    Currently repeatedly used Java classes and methods are cached in the extension modules, by
 *    calling helper functions that return Java objects (jclass, jmethodID, jstring) maybe the
 *    caching could also happen here, so that no Java/JNI types are exposed to outside code at all
 *
 *  - All of the helper functions calling a Java objects's method are wrapped themselves
 *    by another helper function  as well to perform tasks that are common to all of those, i.e.:
 *      jh_call attaches the thread to JVM, instantiates the required Java object,  calls one of
 *      jh_callVoidMethod, jh_callBooleanMethod, jh_callJsonStringMethod, ... (passed as function
 *      pointer to jh_call), deletes the reference to the Java object and returns the PyObject
 *      pointer received from the wrapped function
 *
 *   - Currently we always attach the thread to JVM, because attaching when we don't have to is a
 *     no-op, whereas not attaching when we have to is a big problem
 ...
 *
 * <Arguments>
 *   class_name - Name of a Java Class (const char*), e.g. "com/snakei/SensorService"
 *
 * <Returns>
 ...
 *  - I'd like this not to be exposed to the extensions module (see doc string on top)
 *
 */
...
/*
 * Takes a Java Class reference and the JNI Method signature of a static Java Method
 * (usually a Singleton getter) and returns the Java method reference
 *
 * <Arguments>
 *   class - Native reference to a Java Class (jclass)
 *   type_signature - JNI Method signature (const char*), e.g. "()Lcom/snakei/SensorService;"
 *
 * <Returns>
...
/*
 * Takes a Java Class reference, the name of a Java Object Method and the JNI Method signature
 * of a Java Object Method and returns the Java Method reference
 *
 * <Arguments>
 *   class - Native reference to a Java Class (jclass)
 *   method_name - Java Method name (const char*), e.g. "getSensorList"
 *   type_signature - JNI Method signature (const char*), e.g. "()Ljava/lang/String;"
 *
 * <Returns>
...
/*
 * Takes a Java Class reference, the name of a static Java Method and the JNI Method signature
 * of a static Java Method and returns the Java Method reference
 *
 * <Arguments>
 *   class - Native reference to a Java Class (jclass)
 *   method_name - Java Method name (const char*), e.g. "logMessage"
 *   type_signature - JNI Method signature (const char*), e.g. "(Ljava/lang/String;)V"
 *
 * <Returns>
 *   method - Native reference to a Java Method (jMethodID)
 *
...
/*
 * Takes a Java Class reference and a Java Method reference (usually to a Singleton getter)
 * and returns a Java Object reference
 *
 * <Arguments>
 *   class - Native reference to a Java Class (jclass)
 *   getter - Native reference to a Java Method (jmethodID)
 ...
 *
 * This is useful if we need to pass a Java String to one of the methods wrapped by
 * jh_call which takes variadic arguments.
 *
 * <Arguments>
 ...
 * the native process that was called FROM Java to e.g. start a Python Interpreter/
 * Seattle sandbox in which e.g. native calls to the JVM are performed, returns
 *
 ...
 * calls the toString method of the Java Exception, converts the returned Java String
 * to a Python String and raises a Python Exception with that String. The raised Exception
 * can subsequently handled in Python.
 * Also takes a C char pointer and writes it to the log, to know which C function call
 * was called before Java threw the Exception
 *
 * <Arguments>
 *   jni_env - Pointer to the JNI environment this thread is attached to (JNIEnv*)
 ...
 *
 * If an exception was thrown in Java it gets caught in C and re-raised in Python
 *
 * <Arguments>
 *   jni_env - Pointer to the JNI environment this thread is attached to (JNIEnv*)
 ...
 *
 * Note:
 *   Does not need to be called directly, but rather passed as function pointer to the
...
/*
 * Calls boolean Java Method on passed Java object passing variadic arguments and converts
 * returned Java boolean to Python bool
 *
 * If an exception was thrown in Java it gets caught in C and re-raised in Python
 *
 * <Arguments>
...
/*
 * Calls int Java Method on passed Java object passing variadic arguments and converts
 * returned Java int to Python int
 *
 * If an exception was thrown in Java it gets caught in C and re-raised in Python
 *
 * <Arguments>
...
/*
 * Calls String Java Method on passed Java object passing variadic arguments and converts
 * returned Java String to Python string
 *
 * If an exception was thrown in Java it gets caught in C and re-raised in Python
 *
 * <Arguments>
 ...
 *
 * <Returns>
 *   Python String or Python None (Py_None) if Java returned NULL or NULL if Java threw an exception
 *
 * Note:
 *   Does not need to be called directly, but rather passed as function pointer to the
 *   wrapper jh_call for this method that takes care of attaching thread to JVM
...
PyObject* jh_callStringMethod(JNIEnv* jni_env, jobject object, jmethodID method, va_list args) {
    ...
    // We have to check for NULL, because Java could have returned NULL without throwing an
 ...
 * returned Java String first to C chars, then uses cjson to decode C chars as Python Object
 *
 * If an exception was thrown in Java it gets caught in C and re-raised in Python
 *
 * <Arguments>
 ...
 *
 * Takes a native reference to a Java Class, a native reference to a Java Class Constructor
 * (usually a Singleton constructor) of that class, a function pointer to one of the above
 * helper functions that should be used to call the Java Method, a native reference to that Java
 * Method and variadic arguments.
 *
 * Attaches the current thread to the JVM, then instantiates the Java Object to be called,
 * converts the variadic arguments to a va_list and calls the passed Java Method using
 * the passed helper function on the instantiated Object passing it the va_list as arguments,
 * eventually delete the native reference to the Java Object
 *
 * <Arguments>
 *   class - Native reference to the Java class of the Method to be called (jclass)
 *   get_instance - Native reference to the Constructor of that class (jmethodID)
 *   _jh_call - Function pointer to the helper that is used to call the Java Method
 *   cached_method - Native reference to the Java Method to be called (jmethodID)
 *   ... - Variadic arguments to be passed on to the Java Method
 *
 * <Returns>
 ...
 *
 * Note:
 *  This function has no wrapper like the Java Object Methods above, because so far there
 *  is only one call to a static Java Method in the code that uses these helpers and above all
 *  static functions don't require an object that has to be instantiated beforehand and released
 *  afterwards
 *
 */
