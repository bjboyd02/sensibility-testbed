...
int cache_and_register_stuff(JavaVM* vm) {
  ...
  // XXX (*vm)->AttachCurrentThread(vm, &jni_environment, NULL);

  // PythonInterpreterService is where we register our C functions
  // XXX python_interpreter_service_class = (*jni_environment)->FindClass(jni_environment, "com/snakei/PythonInterpreterService");
  ...
  */
  ...
  // XXX output_service_class = (*jni_environment)->FindClass(jni_environment, "com/snakei/OutputService");
  /* XXX log_message = (*jni_environment)->GetMethodID(jni_environment, output_service_class, 
...
void get_environment(JavaVM* vm) {
  /* Attach the current thread to the Java VM so we can 
   * get the environment for looking up classes. */
  LOGI("Attaching current thread");
  // XXX (*vm)->AttachCurrentThread(vm, &jni_environment, NULL);
  LOGI("Done, environment initted");
}
